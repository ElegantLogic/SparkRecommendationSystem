...
def startGUI():
    ...
    newUserRatings=[]
    #m = open('/home/node/datasets/ml-latest-small/movies.csv').readlines()
...
def recommend():
    spark = SparkSession.builder.config("spark.app.name","Electronics Recommendation System 8m").getOrCreate()
...
# Transform new_user_recommendations_RDD into pairs of the form (Movie ID, Predicted Rating)
    ...
    top_movies = new_user_recommendations_rating_title_and_count_RDD.filter(lambda r: r[2]>=1).takeOrdered(5, key=lambda x: -x[1])
...
class Spark_Recommendation_System: 
    def __init__(self, top=None):
        ...
        font12 = "-family Tahoma -size 24 -weight normal -slant roman "  \
            "-underline 0 -overstrike 0"
        font9 = "-family {Trebuchet MS} -size 16 -weight normal -slant"  \
            " roman -underline 0 -overstrike 0"
        font13 = "-family Tahoma -size 20 -weight normal -slant roman "  \
            "-underline 0 -overstrike 0"
        ...
        #f = open('/home/node/datasets/ml-latest-small/movies.csv')#,encoding='utf-8')
